...
out.append(
    f"{'braco':16} {'n':>2} {'det_found':>9} {'det_sec':>7} {'coer':>5} {'prior':>5} {'N1':>3} {'N2':>3}"
)
A = {}
for arm in ARMS:
    ...
    A[arm] = a
    out.append(
        f"{arm:16} {a['n']:>2} {a['det_found']:>9} {a['det_sec']:>7} {a['coher']:>5} {a['prior']:>5} {a['n1']:>3} {a['n2']:>3}"
    )
...
out.append(
    f"{'modelo':20} " + " ".join(f"{arm.replace('clean-', ''):>9}" for arm in ARMS)
)
for mdl in models:
    cells = []
    for arm in ARMS:
        a = agg(
            rows_for(lambda m, arm=arm, mdl=mdl: m["arm"] == arm and m["model"] == mdl)
        )
...
out.append(
    f"AN - baseline : {round(A['clean-an']['det_found'] - A['clean-baseline']['det_found'], 2)}"
)
out.append(
    f"prosa - baseline: {round(A['clean-prose']['det_found'] - A['clean-baseline']['det_found'], 2)}"
)
out.append(
    f"AN - prosa    : {round(A['clean-an']['det_found'] - A['clean-prose']['det_found'], 2)}"
)
out.append(
    f"P7 found: prosa={A['clean-prose']['found']['P7']}/12 AN={A['clean-an']['found']['P7']}/12 baseline={A['clean-baseline']['found']['P7']}/12"
)
out.append(
    f"P6 found: prosa={A['clean-prose']['found']['P6']}/12 AN={A['clean-an']['found']['P6']}/12 baseline={A['clean-baseline']['found']['P6']}/12"
)

open(os.path.join(tempfile.gettempdir(), "clean_agg.txt"), "w", encoding="utf-8").write(
    "\n".join(out)
)
print("\n".join(out))
