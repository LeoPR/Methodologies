...
def agg(rs):
    n = len(rs)
    found = {
        p: sum(1 for s in rs for x in s["detected"] if x["problem"] == p and x["found"])
        for p in PROBS
    }
    return {
        "n": n,
        ...
        "coh": sum(1 for s in rs if s["coherent"]),
        "found": found,
    }
...
o.append(
    f"{'braco':16} {'n':>2} {'det_found':>9} {'det_sec':>7} {'prior':>5} {'N1':>3} {'N2':>3} {'coer':>4}"
)
A = {}
for arm in ARMS:
    a = agg(rows(lambda m, arm=arm: m["arm"] == arm))
    A[arm] = a
    o.append(
        f"{arm:16} {a['n']:>2} {a['df']:>9} {a['ds']:>7} {a['pr']:>5} {a['n1']:>3} {a['n2']:>3} {a['coh']:>4}"
    )
...
o.append(
    f"  NUVEM : baseline {A['cloud-baseline']['df']} | prosa {A['cloud-prose']['df']} | AN {A['cloud-an']['df']}"
)
print("\n".join(o))
open(os.path.join(tempfile.gettempdir(), "agg_cloud.txt"), "w", encoding="utf-8").write(
    "\n".join(o)
)
