...
def agg(rs):
    n = len(rs)
    found = {
        p: sum(1 for s in rs for d in s["detected"] if d["problem"] == p and d["found"])
        for p in PROBS
    }
    sect = {
        p: sum(
            ...
            for s in rs
            for d in s["detected"]
            if d["problem"] == p and d.get("section_cited_correct")
        )
        for p in PROBS
    }
    return {
...
o = []
o.append(
    f"{'braco':16} {'n':>2} {'det_found':>9} {'det_sec':>7} {'prior':>5} {'N1':>3} {'N2':>3}"
)
A = {}
for arm in ARMS:
    a = agg(rows(arm))
    A[arm] = a
    o.append(
        f"{arm:16} {a['n']:>2} {a['df']:>9} {a['ds']:>7} {a['pr']:>5} {a['n1']:>3} {a['n2']:>3}"
    )
...
if all(k in A for k in ("clean-baseline", "clean-curta", "clean-an", "clean-prose")):
    o.append(
        f"  det_found: baseline {A['clean-baseline']['df']} | prosa-longa {A['clean-prose']['df']} | "
        f"prosa-CURTA {A['clean-curta']['df']} | AN {A['clean-an']['df']}"
    )
    o.append(
        f"  AN - curta = {round(A['clean-an']['df'] - A['clean-curta']['df'], 2)}  "
        f"(curta tem ~mesmo tamanho da AN, SEM gates)"
    )
    o.append(
        f"  curta - baseline = {round(A['clean-curta']['df'] - A['clean-baseline']['df'], 2)}"
    )
    o.append(
        f"  P7 fail-open found: baseline {A['clean-baseline']['found']['P7']} | prosa {A['clean-prose']['found']['P7']} | "
        f"curta {A['clean-curta']['found']['P7']} | AN {A['clean-an']['found']['P7']}"
    )
print("\n".join(o))
open(os.path.join(tempfile.gettempdir(), "agg_all.txt"), "w", encoding="utf-8").write(
    "\n".join(o)
)
