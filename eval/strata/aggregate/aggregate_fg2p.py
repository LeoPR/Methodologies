...
o = []
o.append(
    f"{'braco':22} {'n':>2} {'genuino':>7} {'aluc':>5} {'crit.bom':>8} {'prior':>5}"
)
for arm in ARMS:
    rs = rows(arm)
    n = len(rs)
    o.append(
        f"{arm:22} {n:>2} {round(sum(s['genuine_found'] for s in rs) / n, 2):>7} "
...
print("\n".join(o))
open(os.path.join(tempfile.gettempdir(), "agg_fg2p.txt"), "w", encoding="utf-8").write(
    "\n".join(o)
)
