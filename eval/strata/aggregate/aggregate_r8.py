...
o = []
o.append(
    f"{'braco':22} {'n':>2} {'RI1_dup':>7} {'RI2_ver':>7} {'genuino':>7} {'aluc':>5} {'crit.bom':>8} {'prior':>5}"
)
A = {}
for arm in ARMS:
    rs = rows(arm)
    A[arm] = rs
    o.append(
        f"{arm:22} {len(rs):>2} {rate(rs, 'ri1_dupes'):>7} {rate(rs, 'ri2_version'):>7} "
...
o.append(
    "  genuino=outros problemas reais (media); aluc=problemas FALSOS inventados (media); crit.bom=criticou DIARIO/CHANGELOG bons."
)
o.append(
    "\nLEITURA: o metodo (AN/prosa) faz achar RI1 mais que o baseline? gera mais ou menos alucinacao?"
)
print("\n".join(o))
open(os.path.join(tempfile.gettempdir(), "agg_r8.txt"), "w", encoding="utf-8").write(
    "\n".join(o)
)
