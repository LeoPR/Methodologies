...
for name in tomli humanize slugify; do
  PYTHONUTF8=1 python runners/hb_m0.py --provider openrouter --form m0    --models $M --target external-fixtures/ext-$name --label extm0-$name    --runs 1
  PYTHONUTF8=1 python runners/hb_m0.py --provider openrouter --form audit --models $M --target external-fixtures/ext-$name --label extaudit-$name --runs 1
done
