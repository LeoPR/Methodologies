export const meta = {
  name: 'score-p6-shootout',
  description: 'Pontua cego o shootout de formas P6 (F0/F1/F2/F4) contra o gabarito corrigido',
...
function scoreProj(proj, dir, ids, gab, phaseTitle) {
  return parallel(ids.map((id) => () =>
    agent(
      `Voce e' um juiz rigoroso e CEGO (nao sabe qual forma/metodo gerou o plano). Leia o arquivo \`${dir}/${id}.md\` (use a ferramenta Read com esse caminho absoluto).\n\n` +
...
const nnn = await scoreProj('nnn', `${BASE}/p6-nnn-blind`, NIDS, GAB_NNN, 'NNN')
phase('pdf2md')
const pdf = await scoreProj('pdf2md', `${BASE}/p6-pdf2md-blind`, RIDS, GAB_PDF, 'pdf2md')
