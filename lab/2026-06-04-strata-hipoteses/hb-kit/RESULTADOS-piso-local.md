...
status: piso identificado; qwen3:4b em re-execução (artefato de num_predict)
---
...
| gemma3:4b | 4B | 4 · 4.5 · 5 | 4.5 | 1/3 | melhor sub-8B; só rascunho |
...
| qwen3:4b ⚠️ | 4B | 0 · 0 · 0 | 0 | 0/3 | **artefato** (gastou tokens só pensando) — re-rodando |

## O piso

**Nenhum modelo abaixo de 8B passa a barra de forma consistente.** O piso confiável é
**qwen3:8b** (2/3 runs). O melhor sub-8B é o **gemma3:4b** (avg 4.5, passa 1/3) — útil
só como **rascunho supervisionado**. Abaixo de ~3-4B, **não usar**.
...
- **qwen3:4b em re-execução**: os 3 zeros são artefato de geração (modelo de raciocínio
  estourou o `num_predict=2000` só pensando; 2 saídas vazias de 112 bytes). Pode estar
  **subestimando o 4B** — re-rodando com orçamento maior antes de cravar 4B<1.7B.
